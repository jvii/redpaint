...
// docs/brush-transforms.md, "Sizing a built-in brush"). DPaint regenerated
// its round/square pens procedurally on resize rather than bitmap-stretching
// them (CURBRUSH.C: RoundPen/SquarePen), which is why they stayed crisp at
// any size — these mirror that instead of routing through the pixel-art
// nearest-neighbor `resize()` used for custom brushes.
//
...
// `resize()` made explicit; kept as its own generator so all three families
...
// The dither family (dither3x3/dither7x6) is a hand-placed sparse scatter,
// not a shape that can be stretched to an arbitrary size without turning to
// mush. DPaint didn't try: dragging its DOT_B pen snapped to one of 6 fixed
// bitmaps (CURBRUSH.C: dots10..dots60, decoded below from the original
// Amiga bitplane data in docs/reference/dpaint-source/src/DOTBMS.C — 16-bit
// words, MSB-first, sizes 4s+1 for s=1..6). Resizing a dither brush here
// snaps to the nearest of these the same way.
