// The active cycle range, DPaint's rule: the first PaletteRange containing the
// current foreground color; if the FG color is in no range (or is a literal RGB
// pick with no palette id at all) the whole palette acts as the range. Returns
// 0-based storage indices for the shaders; app-level color ids are 1-based. See
// docs/effects.md.
...
// A contiguous span of palette slots plus its color-cycling settings. DPaint's
// Palette Window had four of these; ours defaults to six (MIN_RANGE_SLOTS) and
// grows past that when a loaded IFF carries more CRNG chunks. The chunk is
// repeatable and real files exceed four. Gradient Fill and the
// Shade/Blend/Cycle paint modes key off start/end only; rate/active/reverse
// belong to color cycling (docs/color-cycling.md).
...
// DPaint's CRNG ranges (an IFF ILBM concept, see src/fileformat/ilbm.ts) become
// range slots (color ids are 1-based where CRNG positions are 0-based). Every
// usable chunk is kept (no cap), and the list is padded to the six default
// slots. DPaint writes unset slots as degenerate low >= high entries; those are
// dropped.
