...
// Rendered only when there *is* one to show. The caller (Menubar) leaves it out
// entirely when a fill would come out solid, where the palette's own foreground
// indicator already tells you the whole story. That absence is DPaint's
// behavior too, not just a simplification; see useFillStyleSwatch's header for
// the DP2 manual's wording on both points.
export function ColorFillBox(): JSX.Element {
  ...
  // physical size divided by the main canvas's live displayScale, which already
  // folds in devicePixelRatio, the screen format's pixel aspect and the current
  // zoom. A pattern therefore tiles here at exactly the size it tiles on the
  // page. Floored at 1 so a deeply zoomed-in canvas can't ask for a
  // zero-dimension buffer.
