...
// picking another tool) needs no restore. They also all operate on the
// current brush only when it's a real CustomBrush, clear the overlay on
// leave/exit, and swallow the context menu so a right-click can't open one
// mid-drag.
export abstract class BrushTransformTool implements Tool {
  ...
  // The current brush, or null when it isn't one these tools can transform
  // (the primitive PixelBrush, before anything has been captured or picked).
  ...
  // jump the brush — inside a bounds box, so the armed mode stays visible
  // even where the brush bitmap is sparse or transparent. (No symmetry: a
  // transform targets the brush itself, not the canvas.)
  //
  // Bend overrides this: its grip is the middle of the edge that will bend,
  // not the brush's center.
