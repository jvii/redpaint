...
// MODES.C): right-click a built-in brush icon, then drag on canvas to resize
// it. Structurally the same anchor/drag/preview interaction as
// StretchBrushTool, but the built-in's family shape is regenerated at each size
// (createSizedBuiltInBrush) instead of bitmap-resampling the small fixed art.
// The same split DPaint had between SizePen (procedural pens) and Stretch
// (STRETCH.C, custom brushes only). No bounds-box overlay here, unlike Stretch:
// Canvas.tsx's offset resize cursor already reads as "you're in this mode"
// (StretchBrushTool.tsx needs the box precisely because its cursor sits on the
// pointer, uncleared), and a built-in's shape is its own outline, so a second
// rectangle around it is redundant.
...
// scalar from the drag (`siz = MAX(ABS(mx-bp.x), ABS(my-bp.y))`,
// MODES.C:pnDnMv), so its round/square pens only ever came out as perfect
// circles/squares, never ellipses/rectangles, regardless of drag direction. The
// ABS() (not a directional delta clamped at a floor) is what gives DPaint's
// resize its "growing again the other way" feel: size is the distance from the
// anchor, so dragging back through it doesn't bottom out, it passes through the
// minimum and grows again on the other side, mirrored. No Shift-to-keep-aspect
// needed here (unlike StretchBrushTool) since the aspect is always locked, not
// just optionally.
