...
// (single-color) gradient range, Pattern mode with nothing captured, or (the
// only other case, polygon only) more vertices than MAX_FILL_POLYGON_VERTICES.
// That last case is rare enough not to warrant a real per-pixel CPU gradient
// fallback (that was the old bucketPointsByGradient path, deleted along with
// it): it's simply painted flat at rangeLow, same as a degenerate range.
...
export function drawStyledFilledShape(
  ...
  // A polygon past the shader's fixed-size loop can't take either GPU path (see
  // MAX_FILL_POLYGON_VERTICES): checked once here rather than in both.
