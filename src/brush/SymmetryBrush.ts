...
export class SymmetryBrush implements BrushInterface {
  ...
  // only appear on release (SYMUP). A no-op unless the shape copies are being
  // suppressed, since otherwise they are already visible. Called from the shape
  // tools' overlay drag branches, the only place the pointer position is
  // known.
  ...
  // DPaint's SYMUP behavior (PAINTW.C): with a captured or loaded stamp brush,
  // the overlay shape preview shows only the primary copy and the full
  // symmetric set appears on release. Stamping a whole shape's worth of copies
  // times 2N per mouse move, each with the full-canvas re-render a draw call
  // costs, is the heaviest preview path in the app, slow enough to invite a GPU
  // context loss in Safari. Everything cheaper stays fully mirrored: the hover
  // cursor, filled shapes, the built-in brushes and pixel-brush previews.
