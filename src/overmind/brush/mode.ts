...
// Modes that show (and paint through, for Matte/DrawImageIndexer-routed modes)
// the pristine matte bitmap rather than the FG-colorized one. The two modes
// where a captured brush's own color is the point (Matte previews its
// transparency; Repl stamps the matte bitmap with holes filled from BG). Every
// other mode's overlay cursor shows the colorized brush, even though the
// canvas-reading effects only ever read its alpha as a shape mask.
